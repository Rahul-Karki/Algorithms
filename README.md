...
|-----------|------|-------|------|
| Power | O(log n) | O(log n) | Recursion |
| Permutations | O(n×n!) | O(n) | Backtracking |
| Tower of Hanoi | O(2^n) | O(n) | Recursion |
| Linear Search | O(n) | O(n) | Search |
| Binary Search | O(log n) | O(log n) | Search |
| Bubble Sort | O(n²) | O(n) | Sorting |
| Merge Sort | O(n log n) | O(n) | Sorting |
| Quick Sort | O(n log n) avg | O(log n) | Sorting |
| Dijkstra | O((V+E) log V) | O(V+E) | Graph |
| Kruskal | O(E log E) | O(V+E) | Graph |
| Floyd-Warshall | O(V³) | O(V²) | Graph |
| LCS | O(m×n) | O(m×n) | DP |
| TSP | O(n²×2^n) | O(n×2^n) | DP |
